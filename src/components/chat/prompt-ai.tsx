...
import { RemixiconComponentType, RiAttachmentLine, RiMic2Line, RiSendPlaneLine } from "@remixicon/react";
...
const PromptAi = ({ selectedCategory, showSuggestions, form, isPending, handleCategoryClick, handleKeyPress, handleSend, handleSuggestionClick, handleTextareaChange, stopRequest, setSelectedCategory, }: Props) => {

    return (
        <div className=' fixed bottom-0 right-0 left-0 bg-[#262626]'>
            <div className="space-y-8 max-w-4xl w-full mx-auto pb-30">
                {showSuggestions && <GreetUser />}
                <div className="relative w-4xl shadow rounded-2xl p-4 space-y-2 bg-muted-foreground/15">
                    <div className="">
                        <Controller
                            name="prompt"
                    ...
                    <div className="relative flex items-center justify-between text-muted">
                        ...
                        <div className="flex items-center">
                            <button disabled={isPending} className='h-10 w-10 flex items-center justify-center text-white rounded-md hover:bg-muted-foreground/20 transition-colors'>
                            ...
                            {form.watch("prompt")?.trim() ? (
                                ...
                                <button
                                    ...
                                    className={`h-10 w-10 flex items-center justify-center text-white rounded-md ${isPending && 'bg-muted-foreground/20 animate-pulse'} hover:bg-muted-foreground/20 transition-colors`}
                                >
                                    {!isPending && <RiMic2Line className='size-6' />}
                                    {isPending && <FaStop className='size-6' />}
                                </button>
                            )}
                        </div>
