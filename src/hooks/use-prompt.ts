...
export const usePrompt = ({ chatUUID, setMessages, messages }: Props) => {
    const [lastMessageUser, setLastMessageUser] = React.useState<Content | undefined>(undefined)
    ...
    const mutation = useMutation({
        ...
        onError: (error: AxiosError) => {
            ...
            setMessages((prev) =>
                prev.map((m) => {
                    if (m.pending) {
                        return { ...m, pending: false, error: "Falha ao obter resposta, tente novamente." }
                    }
                    if (m.role === "USER" && lastMessageUser && m.uuid === lastMessageUser.uuid) {
        ...
        onSuccess: (res) => {
            ...
            if (modelReply) {
                setMessages((prev) =>
                    prev.map((m) =>
                        m.pending
                            ? { ...m, content: modelReply.content, pending: false, isWriting: true }
                            : m
                    )
                );
            }
    ...
    function handleRetry(failedMessageId: string) {
        ...
        const userMessage = failedMessage.role === "USER"
            ? failedMessage
            : messages.slice(0, failedIndex).reverse().find(m => m.role === "USER");
        ...
        setMessages(prev => {
            ...
            const targetIndex = newMessages.findIndex(m => m.uuid === failedMessageId);
    ...
    function handleTypewriterComplete() {
        ...
        setMessages(prev => 
            prev.map(m => 
                m.isWriting ? { ...m, isWriting: false } : m
            )
        )
    }
